...
class ImageProcessor:
    ...
    def process_image(self, path):
        ...
        cv2.imwrite(os.path.join(self.config.output_base_path, 'aligned', f'{name}_aligned_img.png' if not inverted else f'inverted_{name}_aligned_img.png'), aligned_image[0] * 255.)
        segmented_image = self.nets["segmentation_model"].predict(aligned_image)
        cv2.imwrite(os.path.join(self.config.output_base_path, 'segmented', f'{name}_segm_img.png' if not inverted else f'inverted_{name}_segm_img.png'), segmented_image[0] * 255.)
        return {
