...
class TableProcessor:
    def __init__(self, config) -> None:
        ...
        self.train_data.drop(["Hospital","Row_number", "ImageFile"], axis=1, inplace=True)
        self.continuous_features = ["Age","Temp_C","DaysFever","WBC","RBC","CRP","Fibrinogen","Glucose","PCT","LDH","INR","D_dimer","Ox_percentage","PaO2","SaO2","PaCO2","pH"]
        self.discrete_features = ["Sex","PositivityAtAdmission","Cough","DifficultyInBreathing","CardiovascularDisease","IschemicHeartDisease","AtrialFibrillation","HeartFailure",
    ...
    def line_impute_population_average(self, test_data_row):
       ...
       value of the corresponding column in the training data.
        ...
        test_data_row[self.continuous_features] = (test_data_row[self.continuous_features] - self.train_data[self.continuous_features].mean())/self.train_data[self.continuous_features].std()
    ...
    def line_impute_population_sampling(self, test_data_row):
        ...
        for _, col_name in enumerate(test_data_row.columns.values):
            if test_data_row[col_name].isnull().values.any():
                if col_name in self.continuous_features or col_name in self.discrete_features:

                    test_data_row[col_name] = np.random.choice(self.train_data[col_name].dropna())
